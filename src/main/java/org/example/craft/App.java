...
public final class App {
  ...
  public void run(String githubUsername, String freshdeskSubdomain) throws Exception {
    ...
    FreshdeskClient freshdeskClient =
        new FreshdeskClient(httpTransport, freshdeskSubdomain, freshdeskToken);

    execute(githubUsername, githubClient, freshdeskClient);
  }

  public void execute(
      String githubUsername, GitHubClient githubClient, FreshdeskClient freshdeskClient)
      throws Exception {
    GitHubUser githubUser = githubClient.getUser(githubUsername);
    FreshdeskContact freshdeskPayload = Mapper.map(githubUser);

    FreshdeskContact existingContact =
        freshdeskClient.findByExternalId(freshdeskPayload.getUniqueExternalId());
    if (existingContact == null) {
      ...
      String updatedId =
          freshdeskClient.update(String.valueOf(existingContact.getId()), freshdeskPayload);
